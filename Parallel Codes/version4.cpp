...
int numerator ;
int denominator  ;
}__attribute__( ( aligned ( 64 ) ) ) typedef data;
...
double PermComm(igraph_t * g, igraph_vector_t *neig_v, int * vertex_community, int myComm)
{
    ...
    loop_domain_2d *loop2d = new loop_domain_2d(igraph_vector_size (neig_v),igraph_vector_size (neig_v));
	data* workers_data = (data*) malloc(sizeof(data)*get_num_workers());
    
    for(int i=0;i<get_num_workers();i++){
        workers_data[i].denominator=0;
        workers_data[i].numerator=0;
    }
    finish([&](){
        forasync2D(loop2d,[&](int i,int j){
            if(i>=j){return;}
            int neig1 = VECTOR(*neig_v)[i];
            int neig2 = VECTOR(*neig_v)[j];
            if(vertex_community[neig1]!=myComm || vertex_community[neig2]!=myComm){return;}
            int hasEdge = 0;
            int *temp = &hasEdge;
            workers_data[get_current_worker()].denominator++;
            igraph_get_eid(g, temp, neig1, neig2, 0, 0);
            if((*temp)!=-1){
                workers_data[get_current_worker()].numerator++;
            }
        },false,FORASYNC_MODE_RECURSIVE);//End of For Async
    });// End of Finish
    
    
	for(int i=0;i<get_num_workers();i++){
        numerator+=workers_data[i].numerator;
        denominator+=workers_data[i].denominator;
    }
